...
void StringManager::print(std::ostream& os, uint64_t id)
{
    ...
    if (id < static_buffer_size) {
        ...
        len = get_string_len(ptr, &bytes_for_len);
        ptr += bytes_for_len;
...
uint64_t StringManager::get_or_create(const char* str, uint64_t str_len)
{
    ...
    auto bytes_for_len = write_encoded_strlen(len_buf, str_len);
    ...
    if (new_id < static_buffer_size) {
        // write in static_buffer
        memcpy(static_buffer + new_id, len_buf, bytes_for_len);
        auto bytes_to_write = str_len;
        if (new_id + bytes_for_len + str_len >= static_buffer_size) {
            bytes_to_write = static_buffer_size - (new_id + bytes_for_len);
        }
        memcpy(static_buffer + new_id + bytes_for_len, str, bytes_to_write);
    } else {
        // edit first block if it is in dynamic buffer
        // remaining blocks cannot be in dynamic buffer, because they are new
        uint64_t first_block_number = new_id / BLOCK_SIZE;
        std::unique_lock<std::mutex> lock(frame_mutex);

        auto it = frame_map.find(first_block_number);
        if (it != frame_map.end()) {
            Frame* frame = it->second;
            uint64_t offset = new_id % BLOCK_SIZE;
            memcpy(static_buffer + offset, len_buf, bytes_for_len);
            auto bytes_to_write = str_len;
            if (offset + bytes_for_len + str_len >= BLOCK_SIZE) {
                bytes_to_write = BLOCK_SIZE - (offset + bytes_for_len);
            }
            memcpy(frame->bytes + bytes_for_len + offset, str, bytes_to_write);
        }
    }
...
StringManager::Frame& StringManager::get_block(uint64_t block_id)
{
    ...
    if (it == frame_map.end()) {
        ...
        auto read_res = pread(str_file_id.id, frame.bytes, BLOCK_SIZE, block_id * BLOCK_SIZE);
        if (read_res == -1) {
