...
void SMTAutomaton::rename_and_merge(SMTAutomaton& other) {
    ...
    for (size_t i = 0; i < other.from_to_connections.size(); i++) {
        for (auto& t : other.from_to_connections[i]) {
            ...
            // TODO: transition need to receive formulas
            auto transition = SMTTransition(
...
void SMTAutomaton::add_epsilon_transition(uint32_t from, uint32_t to) {
    // TODO: transition need to receive formulas
    add_transition(SMTTransition(from , to, "", false));
}


void SMTAutomaton::transform_automaton(std::function<ObjectId(const std::string&)> f) {
...
void SMTAutomaton::delete_mergeable_states() {
            ...
            if (from_to_connections[state][transition_n].type.empty()) {
                ...
                for (const auto s : epsilon_closure) {
                    for (const auto& t : from_to_connections[s]) {
                        // Connect only if it is not epsilon transition. Always a state
                        // s connect with another state by a non epsilon transition.
                        if (!t.type.empty()) {
                            // TODO: transition need to receive formulas
                            add_transition(SMTTransition(state, t.to, t.type, t.inverse));
                        }
                    }
                }
...
void SMTAutomaton::set_final_state() {
        ...
        for (size_t i = 0; i < from_to_connections.size(); i++) {
            for (size_t j = 0; j < from_to_connections[i].size(); j++) {
                ...
                if (end_states.find(t.to) != end_states.end()) {
                    ...
                    // TODO: transition need to receive formulas
                    add_transition(SMTTransition(t.from, final_state, t.type, t.inverse));
                }
            }
        }
    }
...
void SMTAutomaton::delete_absortion_states() {
...
set<uint32_t> SMTAutomaton::get_epsilon_closure(uint32_t state) {
...
set<uint32_t> SMTAutomaton::get_reachable_states(uint32_t source, bool inverse) {
...
void SMTAutomaton::merge_states(uint32_t destiny, uint32_t source) {
    ...
    for (const auto& t : from_to_connections[source]) {
        if (t.from == t.to) {
            // TODO: transition need to receive formulas
            add_transition(SMTTransition(destiny, destiny, t.type, t.inverse));
        } else {
            // TODO: transition need to receive formulas
            add_transition(SMTTransition(destiny, t.to, t.type, t.inverse));
        }
        // Delete source=[x]=>v
    ...
    for (const auto& t : to_from_connections[source]) {
        if (t.from == t.to) {
            ...
            // TODO: transition need to receive formulas
            add_transition(SMTTransition(t.from, destiny, t.type, t.inverse));
        }
        // Delete v=[x]=>source
...
void SMTAutomaton::calculate_distance_to_final_state() {
...
void SMTAutomaton::sort_transitions() {
...
void SMTAutomaton::sort_state_transition(uint32_t state) {
