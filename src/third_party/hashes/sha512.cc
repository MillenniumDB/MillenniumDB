// Source: https://github.com/pr0f3ss/SHA
// Modified array usage to prevent compiler warnings
/** @file
...
void SHA512::process(uint64** buffer, size_t nBuffer, uint64* h){
	std::vector<uint64> s(WORKING_VAR_LEN);
	std::vector<uint64> w(MESSAGE_SCHEDULE_LEN);

	memcpy(h, hPrime, WORKING_VAR_LEN*sizeof(uint64));

	for(size_t i=0; i<nBuffer; i++){
		// copy over to message schedule
		w.assign(buffer[i], buffer[i] + SEQUENCE_LEN);
