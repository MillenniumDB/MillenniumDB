...
bool DijkstraEnum::next() {
    ...
    while (open.size() > 0) {
        ...
        auto state_reached = current_state_has_next(queue_state.state);
...
robin_hood::unordered_node_set<SearchStateDijkstra>::iterator
    DijkstraEnum::current_state_has_next(const SearchStateDijkstra* current_state)
{
...
void DijkstraEnum::assign_nulls() {
