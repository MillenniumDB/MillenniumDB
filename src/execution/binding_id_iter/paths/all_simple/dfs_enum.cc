...
bool DFSEnum::is_simple_path(const SearchStateDFS& path_state, ObjectId new_node) {
...
SearchStateDFS* DFSEnum::current_state_has_next(SearchStateDFS& current_state) {
...
void DFSEnum::set_iter(SearchStateDFS& current_state) {
...
void DFSEnum::analyze(std::ostream& os, int indent) const {
    ...
    os << "Paths::AllSimple::DFSEnum(idx_searches: " << idx_searches << ", found: " << results_found << ")";
}
