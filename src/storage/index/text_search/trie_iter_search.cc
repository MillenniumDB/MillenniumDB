...
static uint16_t inc(uint16_t val, unsigned char a, unsigned char b)
{
    ...
    return (a & 0b1100'0000) == 0b1000'0000 && (b & 0b1100'0000) == 0b1000'0000 ? val : val + 1;
}
        ...
        if (query_idx < query.size() || node_str_idx < node_str_length) {
            ...
            if (query_idx >= query.size()) {
                ...
                if constexpr (type == SearchType::PREFIX) {
                    auto next_matched_len = inc(matched_len, node->string_ptr[node_str_idx]);
                    priority_queue
                        .emplace(node->clone(), query_idx, node_str_idx + 1, next_matched_len, error_count);
                }
                ...
                for (size_t c = 0; c < *node->child_count_ptr; c++) {
                    ...
                    auto child_page_pointer = read_xbytes(
                        child_ptr + Node::CHILD_CHAR_SIZE,
                        Node::CHILD_POINTER_SIZE
                    );
                    ...
                    if (child_char == query[query_idx]) {
                        ...
                        priority_queue
                            .emplace(std::move(child_node), query_idx + 1, 0, next_matched_len, error_count);
                ...
                auto next_matched_len = inc(matched_len, node->string_ptr[node_str_idx]);
                priority_queue
                    .emplace(node->clone(), query_idx + 1, node_str_idx + 1, next_matched_len, error_count);
            }
        } else {
            ...
            if (type == SearchType::PREFIX || allow_errors) {
                for (size_t c = 0; c < *node->child_count_ptr; c++) {
                    ...
                    if constexpr (type == SearchType::PREFIX) {
                        auto next_matched_len = inc(matched_len, child_char);
                        priority_queue
                            .emplace(std::move(child_node), query_idx, 0, next_matched_len, error_count);
                    }
                }
            }
