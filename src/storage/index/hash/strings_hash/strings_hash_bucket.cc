...
uint64_t StringsHashBucket::get_str_id(const char* bytes, uint64_t size, uint64_t hash) const {
...
uint64_t StringsHashBucket::get_or_create_str_id(const char* bytes, uint64_t size, uint64_t hash, bool* need_split) {
    for (size_t i = 0; i < *key_count; ++i) {
        if (hashes[i] == hash) {
            // check if object is
            auto id = ids[i];
    ...
    auto new_id = string_manager.create_new(bytes, size);
