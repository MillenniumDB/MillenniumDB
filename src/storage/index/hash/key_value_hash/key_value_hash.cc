...
void KeyValueHash<K, V>::insert(const std::vector<K>& key, const std::vector<V>& value)
{
    ...
    while (current_bucket_page->get_tuple_count() >= max_tuples) {
        if (tuples_count
            >= get_split_threshold()) { // split threshold should depend on depth or could be infinite
            split();
            mask = (1 << depth) - 1;
...
template<class K, class V>
pair<vector<K>, vector<V>>
    KeyValueHash<K, V>::get_pair(uint_fast32_t bucket_number, uint_fast32_t current_pos)
{
    ...
    auto current_bucket_page = std::make_unique<KeyValueHashBucket<K, V>>(
        buckets_file,
        real_page_number,
        key_size,
        value_size,
...
void KeyValueHash<K, V>::split()
{
    ...
    for (uint_fast32_t bucket_number = 0; bucket_number < previous_number_of_buckets; bucket_number++) {
        ...
        auto new_bucket_page = make_unique<KeyValueHashBucket<K, V>>(
            buckets_file,
            new_page_number,
            key_size,
            value_size,
        ...
        *(new_bucket_page->tuple_count) = 0;
        ...
        auto writing_old_bucket_page = make_unique<KeyValueHashBucket<K, V>>(
            buckets_file,
        ...
        for (auto reading_page_number : buckets_page_numbers[bucket_number]) {
            auto reading_old_bucket_page = make_unique<KeyValueHashBucket<K, V>>(
                buckets_file,
                reading_page_number,
                key_size,
                value_size,
            ...
            for (uint_fast32_t tuple_number = 0; tuple_number < bucket_page_tuples; tuple_number++) {
                ...
                assert(
                    insert_bucket_number == bucket_number
                    || insert_bucket_number == previous_number_of_buckets + bucket_number
                );
...
template<class K, class V>
bool KeyValueHash<K, V>::find_first(
    const std::vector<K>& current_key,
...
void KeyValueHash<K, V>::sort_bucket(uint_fast32_t bucket_number)
{
    ...
    while (buckets_page_numbers[bucket_number].size() > merge_size) {
        ...
        *(current_bucket_page->tuple_count) = 0;
        current_bucket_page->page.make_dirty();
        for (uint_fast32_t step = 0; step < buckets_page_numbers[bucket_number].size();
             step += (2 * merge_size))
        {
            ...
            current_left_page = std::make_unique<KeyValueHashBucket<K, V>>(
                buckets_file,
                buckets_page_numbers[bucket_number][left_page_number],
                key_size,
                value_size,
                max_tuples
            );
            current_right_page = std::make_unique<KeyValueHashBucket<K, V>>(
                buckets_file,
                buckets_page_numbers[bucket_number][right_page_number],
                key_size,
                value_size,
                max_tuples
            );
            ...
            while (left_page_number < step + merge_size && right_page_number < step + (2 * merge_size)
