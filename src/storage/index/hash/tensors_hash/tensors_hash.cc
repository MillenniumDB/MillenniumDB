...
TensorsHash::TensorsHash(const std::string& filename) :
    dir_file_id(file_manager.get_file_id(filename + ".dir")),
    buckets_file_id(file_manager.get_file_id(filename + ".dat"))
{ }

uint64_t TensorsHash::get_bytes_id(const char* bytes, uint64_t size) const
{
...
void TensorsHash::create_bytes_id(const char* bytes, uint64_t size, uint64_t new_id)
{
    uint64_t hash = HashFunctionWrapper(bytes, size);
