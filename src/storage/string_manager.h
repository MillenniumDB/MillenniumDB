...
public:
    ...
    // max_initial_populate_size in GB
    ...
    uint64_t get_bytes_id(const char* bytes, uint64_t size, bool create_if_not_exists = false);

    uint64_t get_str_id(const std::string& str, bool create_if_not_exists = false) {
        return get_bytes_id(str.data(), str.size(), create_if_not_exists);
    }

    // must check the string doesn't exist before calling this method
    uint64_t create_new(const char* bytes, uint64_t size);

    // must check the string doesn't exist before calling this method
    uint64_t create_new(const std::string& str) {
        return create_new(str.data(), str.size());
    }
    ...
    // how many bytes would be used to encode a string with a length of `str_len`.
    static inline uint64_t get_bytes_for_len(uint64_t str_len) {
        uint64_t significant_bits = 64 - __builtin_clzll(str_len);
    ...
    static void write_encoded_strlen(char* buffer, uint64_t str_len) {
        auto* ptr = buffer;
