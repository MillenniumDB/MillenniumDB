...
void ResponseParser::begin(Binding& parent_binding)
{
    ...
    if (response_status != 200) {
        throw runtime_error(
            "Bad Request to " + host + " with HTTP response status code: " + to_string(response_status)
        );
    }
...
bool ResponseParser::parse_iri_and_body(string& host, string& target, string& body)
{
    ...
    if (std::holds_alternative<std::string>(var_or_iri)) {
        ...
        if (oid.generic_type() != ObjectGenType::Iri) {
    ...
    string values_header =
        "VALUES ("; // If query has a VALUES for a join var, 2 VALUES means the intersection (the expected result)
    ...
    for (const auto& [var_name, values] : result->second.get_child("")) {
        ...
        if (found && scope_vars.find(var_id) != scope_vars.end()) {
            ...
            if (attr_type == "literal" && values.count("datatype")) { // checks if there is a datatype child
                ...
                attr_type == "literal" && values.count("xml:lang")
            ) { // checks if there is a language child
...
bool ResponseParser::extract_object_xml()
{
    if (result == results.end())
        return false;
    if (result->first == "<xmlattr>")
        ++result; // sometimes results XML variable has a XML attribute
    if (result == results.end())
        return false;
    ...
    for (auto& attr : result->second.get_child("")) {
        ...
        if (found && scope_vars.find(var_id) != scope_vars.end()) {
            ...
            if (attr.second.count("literal")) { // checks if there is a literal child before extracting it
                ...
                attr.second.count("bnode")
            ) { // checks if there is a blank node child before extracting it
                attr_type = "bnode";
...
bool ResponseParser::header_automata()
{
    ...
    if (pos == string::npos)
        pos = response.length() - 1; // response is not empty
    if (pos == 0)
        return false;
    if (response[pos - 1] == '\r')
        pos -= 1;
...
bool ResponseParser::response_automata()
{
    ...
    for (pos = line_end + 1; pos < response.length(); pos++) {
        ...
        if (c == '\t' || c == '\n' || (pos == response.length() - 1)) {
            ...
            if (c != '\n' && (pos == response.length() - 1) && (in_datatype || in_lang))
                extra_data += c;
            if (c != '\n' && (pos == response.length() - 1))
                attr_value += c;
            ...
            ((in_simple_string && c == '\'') || (in_double_string && c == '\"'))
            && (pos <= response.length() - 4) && (response.compare(pos + 1, 3, "^^<") == 0)
        ) { // 3-lookahead
            in_simple_string = false;
            in_double_string = false;
            in_datatype = true;
            ...
            ((in_simple_string && c == '\'') || (in_double_string && c == '\"'))
            && (pos <= response.length() - 2) && (response.at(pos + 1) == '@')
        ) { // 1-lookahead
            in_simple_string = false;
            in_double_string = false;
            in_lang = true;
...
ObjectId
    ResponseParser::get_object_id(const string& attr_type, const string& attr_value, const string& extra_data)
{
    string new_type = "";
