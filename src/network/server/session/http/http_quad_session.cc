...
std::unique_ptr<Op> HttpQuadSession::create_readonly_logical_plan(const std::string& query) {
...
void HttpQuadSession::execute_readonly_query(const std::string& query, std::ostream& os, MQL::ReturnType response_type) {
...
void HttpQuadSession::execute_update_query(const std::string& query, std::ostream& os) {
    ...
    std::unique_ptr<MQL::OpUpdate> current_logical_plan;
    try {
        // current_logical_plan = create_update_logical_plan(query); // TODO:
    }
    catch (const QueryParsingException& e) {
