...
double EdgePlan::estimate_output_size() const {
    ...
    if (from == to) {
        if (from == type) {
            ...
            if (type.is_OID()) {
                const auto special_connections_with_type = static_cast<double>(
                    quad_model.catalog().equal_from_to_type_with_type(type.get_OID().id)
                );
            ...
            if (type.is_OID()) {
                const auto special_connections_with_type = static_cast<double>(
                    quad_model.catalog().equal_from_to_with_type(type.get_OID().id)
                );
                if (from_assigned) {
        ...
        if (type.is_OID()) {
            const auto special_connections_with_type = static_cast<double>(
                quad_model.catalog().equal_from_type_with_type(type.get_OID().id)
            );
            if (to_assigned) {
                return special_connections_with_type / distinct_to;
            } else {
        ...
        if (type.is_OID()) {
            const auto special_connections_with_type = static_cast<double>(
                quad_model.catalog().equal_to_type_with_type(type.get_OID().id)
            );
            if (from_assigned) {
                return special_connections_with_type / distinct_from;
            } else {
        ...
        if (type.is_OID()) {
            const auto connections_with_type = static_cast<double>(
                quad_model.catalog().connections_with_type(type.get_OID().id)
            );
            if (from_assigned) {
                if (to_assigned) {
...
bool EdgePlan::get_leapfrog_iter(std::vector<std::unique_ptr<LeapfrogIter>>& leapfrog_iters,
                                 vector<VarId>&                              var_order,
                                 uint_fast32_t&                              enumeration_level) const
{
    // TODO: support special cases
    if (   (from.is_var() && from == to)
        || (from.is_var() && from == type)
        || (to.is_var()   && to   == type)
        // TODO: these cases are trivially empty
        || (from.is_var() && from == edge)
        || (to.is_var()   && to   == edge)
        || (type.is_var() && type == edge))
    {
        return false;
    }
    ...
    auto get_iter_from_quad = [&initial_ranges, &enumeration_vars, &intersection_vars]
