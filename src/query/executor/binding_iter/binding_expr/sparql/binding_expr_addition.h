...
public:
    ...
    template<typename T>
    inline ObjectId handle_add_tensors(ObjectId lhs_oid, ObjectId rhs_oid)
    {
        ...
        if (RDF_OID::get_generic_type(rhs_oid) == RDF_OID::GenericType::TENSOR) {
            ...
            return Conversions::pack_tensor<T>(lhs.add(rhs));
        } else {
    ...
    ObjectId eval(const Binding& binding) override
    {
        ...
        case Conversions::OPTYPE_TENSOR_FLOAT: {
            return handle_add_tensors<float>(lhs_oid, rhs_oid);
        }
        case Conversions::OPTYPE_TENSOR_DOUBLE: {
            return handle_add_tensors<double>(lhs_oid, rhs_oid);
        }
