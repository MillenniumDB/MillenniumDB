...
EdgeDescribeStreamingExecutor::EdgeDescribeStreamingExecutor(std::unique_ptr<BindingIter> key_value_iter_,
...
uint64_t EdgeDescribeStreamingExecutor::execute(MDBServer::StreamingResponseWriter& response_writer) {
    ...
    // from, to, type
    response_writer.write_object_id(ObjectId((*record)[0]));
    response_writer.write_object_id(ObjectId((*record)[1]));
    response_writer.write_object_id(ObjectId((*record)[2]));
