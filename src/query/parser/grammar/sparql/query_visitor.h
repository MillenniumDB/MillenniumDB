...
private:
    ...
    MDBFunction::MDBFunctionType current_function_type { MDBFunction::MDBFunctionType::NONE };
    std::vector<std::unique_ptr<Op>> mdb_functions;
    ...
    // Check if an IRI ObjectId is a valid MDB function. This will set the current_function_type.
    // MDBFunction::MDBFunctionType::NONE will be set if the input is not a valid function.
    void check_mdb_function(const IdOrPath& id_or_path);

    // Converts an Object{Path}Context to a vector of Ids
    template <typename ContextTraits>
    std::vector<Id> parse_context_as_function_args(typename ContextTraits::ContextType* ctx);
