...
matchQuery:
    primitiveStatementList whereStatement? groupByStatement? orderByStatement? returnStatement;
...
insertLinearPattern:
    insertPlainNode (insertPlainEdge insertPlainNode)*;

insertPlainNode:
    '(' insertPlainNodeInside? TYPE* properties? ')';
...
// deleteElement: (identifier) (TYPE|KEY)? | EDGE_ID KEY? | VARIABLE KEY? | K_LABELS '(' insertObj
...
// setElement: K_PROPERTIES '(' insertObj ')' '=' insertProperties | K_LABELS '(' insertObj ')' '='
// '{' TYPE (',' TYPE)* '}' | (identifier|VARIABLE) KEY '=' '{' TYPE (',' TYPE)* '}' ;

// insertElement: insertNode (insertEdge insertNode)*;

// insertNode: '(' insertObj? TYPE* insertProperties?')';

// insertEdge: '<' '-' '[' TYPE insertProperties? ']''-' | '-' '[' TYPE insertProperties? ']''-' '>'
// ;

// insertProperties: '{' insertProperty (',' insertProperty)* '}';

// TODO: maybe allow expressions as values? insertProperty2 is necessary when the property is
// written without spaces after the colon, example: key:date("2001-02-03") key :date identifier TYPE
// '(' STRING ')'; insertProperty: identifier (':' value | TRUE_PROP | FALSE_PROP) # insertProperty1
