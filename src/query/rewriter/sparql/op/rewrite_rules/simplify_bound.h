...
private:
    ...
    void reset()
    {
        has_regrouped = return_empty_op = changed_to_join = false;
    }
...
public:
    bool is_possible_to_regroup(std::unique_ptr<Op>& unknown_op) override
    {
        ...
        for (auto& expr : op_filter->filters) {
            ...
            // BndII
            else if (is_castable_to<ExprBound>(expr) && !expr_is_bound_and_is_in_vars(expr, vars))
            {
...
private:
    ...
    bool expr_is_bound_and_is_in_vars(std::unique_ptr<Expr>& expr, const std::set<VarId>& vars)
    {
    ...
    bool expr_not_has_bound_expr_and_is_in_vars(std::unique_ptr<Expr>& expr, const std::set<VarId>& vars)
    {
        if (!is_castable_to<ExprNot>(expr)) {
            return false;
        }
