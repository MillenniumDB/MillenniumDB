...
void OptimizeOptionalTree::visit(OpOptional& op_optional) {
    ...
    if (op_optional.optionals.size() == 0) {
        if (current_move_children_up) {
            ...
            optionals.emplace_back(std::move(op_optional.op));
        }
        ...
        delete_current = false;
        for (auto it=op_optional.optionals.begin(); it!=op_optional.optionals.end(); ) {
            optionals.emplace_back(std::move(*it));
            it = op_optional.optionals.erase(it);
        }
    }
}
...
void OptimizeOptionalTree::visit(OpBasicGraphPattern& op_basic_graph_pattern) {
    ...
    // delete already assigned disjoint vars
    for (auto it = op_basic_graph_pattern.disjoint_vars.begin(); it != op_basic_graph_pattern.disjoint_vars.end(); ) {
        auto op_disjoint_object = *it;

        if (global_vars.find(op_disjoint_object.var) != global_vars.end()) {
            it = op_basic_graph_pattern.disjoint_vars.erase(it);
        } else {
            ++it;
        }
    }
    ...
    if (op_basic_graph_pattern.edges.empty()
        ...
        && op_basic_graph_pattern.disjoint_vars.empty())
    {
        delete_current = true;
    }

    auto match_vars = op_basic_graph_pattern.get_all_vars();
