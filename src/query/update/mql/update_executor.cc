...
void UpdateExecutor::execute(const UpdateContext& update_context)
{
    ...
    for (auto& property_info : update_context.new_properties) {
        ...
        if (existing_record != nullptr) {
            ...
            //             text_index_name2deletes[name].emplace_back(obj, (*existing_record)[2]);
            //             text_index_name2inserts[name].emplace_back(obj, val);
            //         }
            //     }
            // }
            ...
            //         for (const auto& name : it->second) {
            //             hnsw_index_name2deletes[name].emplace_back(obj, (*existing_record)[2]);
            //             hnsw_index_name2inserts[name].emplace_back(obj, val);
            //         }
            //     }
            // }
        } else {
    ...
    for (auto& edge : update_context.deleted_edges) {
        quad_model.edge_from_to_type->delete_record({});

        Record<4> min_range = { edge.id, 0, 0, 0 };
        Record<4> max_range = { edge.id, UINT64_MAX, UINT64_MAX, UINT64_MAX };
        auto prop_iter = quad_model.edge_from_to_type
                             ->get_range(&interruption_requested, min_range, max_range);
        const auto existing_record = prop_iter.next();

        if (existing_record != nullptr) {
            // TODO: delete all edge properties

            quad_model.from_to_type_edge->delete_record(*existing_record);
            quad_model.to_type_from_edge->delete_record(*existing_record);
            quad_model.type_from_to_edge->delete_record(*existing_record);
            quad_model.type_to_from_edge->delete_record(*existing_record);
            ...
            if (from == to) {
                quad_model.equal_from_to->delete_record({ from, type, edge.id });
                quad_model.equal_from_to_inverted->delete_record({ type, from, edge.id });

                if (from == type) {
                    quad_model.equal_from_to_type->delete_record({ from, edge.id });
                }
            }
            if (from == type) {
                quad_model.equal_from_type->delete_record({ from, to, edge.id });
                quad_model.equal_from_type_inverted->delete_record({ to, from, edge.id });
            }
            if (to == type) {
                quad_model.equal_to_type->delete_record({ to, from, edge.id });
                quad_model.equal_to_type_inverted->delete_record({ from, to, edge.id });
            }
        }
    }
